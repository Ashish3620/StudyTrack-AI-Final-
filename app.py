...
# ------------------------------------------------
    ...
    if file:
        ...
        if st.button("Train Model"):
            ...
            st.success("✅ Model trained using Linear Regression")

            st.markdown("### 📊 Model Performance")
            st.write(f"**R² Score (Accuracy):** {r2*100:.2f}%")
            #st.write(f"**Mean Absolute Error (MAE):** {mae:.2f} marks")
...
# ------------------------------------------------
    ...
    if "trained_data" not in st.session_state:
        ...
        #Clustering-------------------------
        st.divider()
        st.subheader("📌 Student Clusters based on Predicted Marks")

        # Safety check
        required_cols = ["Study_Hours", "Predicted_Marks", "Performance_Level"]
        if not all(col in df.columns for col in required_cols):
            st.warning("Required data not available for clustering visualization.")
        else:
            fig = px.scatter(
                df,
                x="Study_Hours",
                ...
                labels={
                    ...
                    "Predicted_Marks": "Predicted Marks"
                }
            )
